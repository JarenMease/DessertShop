...
def main_menu():
    ...
    for item in my_order.order:
        if isinstance(item, Sundae):
            order_formatted.append([item.name + " (" + item.packaging + ")", str(int(item.unit)) + " scoops",
                                    "$" + str(item.price) + "/scoop",
            ...
            order_formatted.append([item.name + " (" + item.packaging + ")", str(int(item.unit)) + " cookies",
                                    "$" + str(item.price) + "/dozen",
                                    "$" + str(round(item.calculate_cost(), 2)),
                                    "$" + str(round(item.calculate_tax(), 2))])
        elif isinstance(item, Candy):
            order_formatted.append([item.name + " (" + item.packaging + ")", str(round(item.unit, 1)) + "lbs",
                                    "$" + str(item.price) + "/lb",
                                    "$" + str(round(item.calculate_cost(), 2)),
                                    "$" + str(round(item.calculate_tax(), 2))])
